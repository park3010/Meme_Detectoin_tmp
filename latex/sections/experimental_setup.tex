...
Following a source-only integrity audit, we exclude 72 redundant,
raw-label-conflicting, or structured-label-conflicting records.  The effective
supervised source pool therefore contains 6,941 samples.  Conflicting duplicate
groups are excluded in full, decoded-image groups are assigned atomically to a
split, and source-confirmed shared-image/different-text records are retained.
...
We construct a single immutable, decoded-image-group-aware source split using
5,552 records for training and 1,389 for validation (approximately 80\%/20\%).
