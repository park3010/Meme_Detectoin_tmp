...
for dataset in "${DATASET_ARRAY[@]}"; do
  for baseline in image_only text_only clip_concat; do
    case "${baseline}" in
      image_only) script="scripts/run_baseline_image_only.py" ;;
      text_only) script="scripts/run_baseline_text_only.py" ;;
      clip_concat) script="scripts/run_baseline_clip_concat.py" ;;
    esac
  ...
  run_with_optional_limit "${PYTHON}" scripts/run_ours_full.py \
    --config "${CONFIG}" --dataset "${dataset}" --seed "${SEED}" \
  ...
  run_cmd "${PYTHON}" scripts/evaluate_structured_outputs.py \
  ...
  for ablation in w_o_retrieval w_o_verifier w_o_task_aware_gate; do
    ...
    run_with_optional_limit "${PYTHON}" scripts/run_ablation.py \
      --config "${CONFIG}" --dataset "${dataset}" --seed "${SEED}" \
