...
from experiments.protocol_v2 import audit_source_split_v2
...
def run_research_preflight(
    *,
    ...
    if str(protocol.get("name")) == "harmeme_to_fhm_v2":
        if force_regenerate_split:
            raise RuntimeError("v2 manifests are resolution-bound and cannot be regenerated by generic preflight")
        source_path = Path(str(protocol.get("source_split_manifest")))
        fhm_path = Path(str(protocol.get("heldout_test_manifest", DEFAULT_FHM_MANIFEST)))
        source_obj = load_manifest(source_path)
        split_audit = audit_source_split_v2(source_obj)
        manifests = {
            ...
            "source": split_audit,
            "fhm": {"passed": fhm_path.is_file(), "preservation_only": True},
        }
    else:
        manifests = ensure_protocol_manifests(
            label_set=str(protocol.get("label_set", "clean")), split_seed=int(protocol.get("split_seed", 42)),
            source_manifest_path=str(protocol.get("source_split_manifest")), fhm_manifest_path=str(protocol.get("heldout_test_manifest", DEFAULT_FHM_MANIFEST)),
    ...
    errors = []
    if not manifests["source"].get("passed"):
        errors.extend(item.get("code", str(item)) for item in manifests["source"].get("errors", []))
